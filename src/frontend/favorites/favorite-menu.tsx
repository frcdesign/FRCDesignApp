...
function FavoriteMenuDialog(
  ...
  const setDefaultConfigurationMutation = useMutation({
      ...
      queryClient.setQueryData(
        ...
        getQueryUpdater((data: LibraryUserData) => {
          if (data.favorites[favoriteId]) {
            data.favorites[favoriteId].defaultConfiguration = configuration;
          }
          return data;
        }),
      );
    ...
    onError: () => {
      showErrorToast("Unexpectedly failed to update default configuration.");
    },
    onSuccess: () => {
  ...
  return (
    <Dialog
      ...
      onClose={closeDialog}
    >
      <PreviewImageCard path={element.path} configuration={configuration} />
      <DialogBody>
