...
import { getUnitDisplayStr, QuantityType, Unit } from "./enums";
...
/*
  ...
  Note the following code is generated by ChatGPT.


  EXP (lowest precedence)
    EXP         ::= POSTFIX { ("+" | "-") POSTFIX }

  POSTFIX (unit application)
    POSTFIX     ::= TERM [ Identifier ]
                    // optional unit name applies to the entire TERM
...
const POSTFIX = rule<TokenKind, Expr>();

PRIMARY.setPattern(
    alt(
        // number (no unit here)
        apply(tok(TokenKind.Number), (numTok) => {
            const valueLiteral: ValueLiteral = {
                value: parseFloat(numTok.text),
                type: "number",
...
FACTOR.setPattern(
    alt(
        // unary +/- binds tightly: +FACTOR or -FACTOR
...
// TERM groups multiplicative sequences (*/ has higher precedence)
TERM.setPattern(
    apply(
...
POSTFIX.setPattern(
    apply(seq(TERM, opt(tok(TokenKind.Identifier))), ([term, idTok]) => {
        if (!idTok) return term;
        const unit = classifyUnit(idTok.text);
        return {
            kind: "unit-application",
            expr: term,
            unit,
...
EXP.setPattern(
    apply(
        seq(POSTFIX, rep_sc(seq(tok(TokenKind.PlusMinus), POSTFIX))),
        ([first, rest]) =>
            rest.reduce<Expr>(
                (acc, [opTok, rhs]) => ({
                    kind: "binary",
