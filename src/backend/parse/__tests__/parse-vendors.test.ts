...
describe("parseVendors", () => {
    ...
    it("returns empty array when name has no vendor and no configuration", () => {
    ...
    it("falls back to enum option names when no name vendor found", () => {
        ...
        const result = parseVendors("Swerve Module", configuration);
    ...
    it("skips non-ENUM parameters", () => {
        ...
        expect(parseVendors("Generic Part", configuration)).toEqual([]);
    });
});
